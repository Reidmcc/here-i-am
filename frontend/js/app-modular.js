...
import {
    ...
    handleGoScore,
    getGoGameContext,
    executeGoMoveFromResponse,
    loadGameForConversation,
...
class App {
    ...
    initializeModules() {
        ...
        setChatCallbacks({
            ...
            getGoGameContext: () => getGoGameContext(),
            executeGoMove: (content) => executeGoMoveFromResponse(content),
        ...
        // Initialize Go game UI module
        setGoGameElements(this.elements);

        // Initialize GoGameController if available
        if (typeof GoGameController !== 'undefined') {
            const goGame = new GoGameController({
                onGameUpdate: (game) => updateGoGameUI(game),
                onError: (error) => showToast(error, 'error'),
            });
            setGoGameController(goGame);

            // Initialize the board
            if (this.elements.goBoardContainer) {
                initGoBoard(this.elements.goBoardContainer);
            }
        }
    }
    ...
    bindEvents() {
        ...
        this.elements.goPassBtn?.addEventListener('click', () => handleGoPass());
        this.elements.goResignBtn?.addEventListener('click', () => handleGoResign());
        this.elements.goScoreBtn?.addEventListener('click', () => handleGoScore());
        this.elements.goCreateGame?.addEventListener('click', () => createGoGame());
        document.getElementById('close-go-new-game')?.addEventListener('click', () => hideGoNewGameModal());
        document.getElementById('cancel-go-new-game')?.addEventListener('click', () => hideGoNewGameModal());
    ...
    onConversationLoaded(conversation, messages) {
        ...
        loadGameForConversation(conversation.id);
