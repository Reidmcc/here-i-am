...
export function addMessage(role, content, options = {}) {
    ...
    // Claude Code conversations are read-only records: edit and regenerate
    // would rebuild a context the original session never had, so only
    // copy/speak are offered
    const isReadOnlyConversation = state.currentConversationSource === 'claude_code';

    if (options.messageId && !options.isError) {
        if (role === 'human') {
            const editBtn = isReadOnlyConversation ? '' : `<button class="message-action-btn edit-btn" title="Edit message"><svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M11 4H4a2 2 0 0 0-2 2v14a2 2 0 0 0 2 2h14a2 2 0 0 0 2-2v-7"/><path d="M18.5 2.5a2.121 2.121 0 0 1 3 3L12 15l-4 1 1-4 9.5-9.5z"/></svg></button>`;
            actionButtons = `<div class="message-bubble-actions">${copyBtn}${editBtn}</div>`;
            ...
            const regenerateBtn = (options.isLatestAssistant && !isReadOnlyConversation) ? `<button class="message-action-btn regenerate-btn" title="Regenerate response"><svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M23 4v6h-6"/><path d="M1 20v-6h6"/><path d="M3.51 9a9 9 0 0 1 14.85-3.36L23 10M1 14l4.64 4.36A9 9 0 0 0 20.49 15"/></svg></button>` : '';
