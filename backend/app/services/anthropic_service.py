...
class AnthropicService:
    ...
    async def send_message(
        ...
        logger.info(f"[CACHE DEBUG] API Response - input: {usage.get('input_tokens')}, output: {usage.get('output_tokens')}")
        logger.info(f"[CACHE DEBUG] Cache write: {usage.get('cache_creation_input_tokens', 0)}, Cache read: {usage.get('cache_read_input_tokens', 0)}")
    ...
    async def send_message_stream(
        ...
        try:
            ...
            # Debug logging for cache results
            logger.info(f"[CACHE DEBUG] Stream API Response - input: {input_tokens}, output: {output_tokens}")
            logger.info(f"[CACHE DEBUG] Cache write: {cache_creation_input_tokens}, Cache read: {cache_read_input_tokens}")
    ...
    def build_messages_with_memories(
        ...
        logger.info(f"[CACHE DEBUG] Old memories: {len(old_memories)}, New memories: {len(new_memories)}")
        logger.info(f"[CACHE DEBUG] Cached text tokens: {cached_tokens} (minimum 1024 required for Sonnet/Opus)")
        logger.info(f"[CACHE DEBUG] Will cache: {enable_caching and bool(cached_text) and cached_tokens >= 1024}")
