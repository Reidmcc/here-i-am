...
- **Recent reflections on first turn:** `RECENT_REFLECTIONS_ENABLED` (default off) injects the `RECENT_REFLECTIONS_COUNT` (default 3) most recently created `memory_save` reflections on a conversation's *first* turn only, alongside semantic retrieval. Selected purely by recency (`memory_service.get_recent_reflections` — SQL on `speaker_entity_id`, no Pinecone), deduplicated against semantic results (reflections stay semantically retrievable), released/archived excluded. Unrelated to the "reflection mode" settings (`reflection_seed_count`).
