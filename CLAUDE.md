...
- **Interrupted tool turns resume, they don't restart:** a turn's progress inside the agentic loop lives on a `PendingToolTurn` (`conversation_session.py`) — completed tool exchanges, text streamed so far, next iteration, and the built `base_messages`. When an iteration fails, two layers apply. (1) A *transient* provider failure (classified by `provider_errors.classify_stream_error`: 429/5xx/529, connection/timeout → retryable; 4xx and anything unrecognized → not) re-runs the same iteration up to `TOOL_LOOP_RETRY_ATTEMPTS` (3) times with exponential backoff. The request is byte-identical, so the retry is a cache read; any text the failed attempt streamed is withdrawn with a `stream_rewind` SSE event (the frontend truncates the bubble to `checkpoint`). (2) If retries are exhausted (or the error is permanent) *and* at least one tool iteration completed, the turn is stashed on the session (`session.pending_tool_turn`) and `{"error_type":"interrupted_tool_turn"}` is yielded — deliberately *without* `add_exchange`/`update_cache_state`, so the context still matches what `base_messages` was built against. The frontend keeps the partial response and tool messages on screen and offers **Resume**, which re-sends with `resume=true` plus the `turn_id` from the error (so a stale Resume button can't consume a *different* stashed turn and persist its human message); the route hands the stash to `session_manager.resume_message_stream`, which skips retrieval/trimming/message-building entirely and re-enters the loop at the failed iteration. Tools that already ran are **not** re-executed (matters for side effects: notes writes, GitHub commits, Moltbook posts). Failing on the *first* iteration stashes nothing and surfaces the raw provider error. `take_resumable_turn` rejects a stash whose `turn_id` doesn't match, whose entity changed, whose context length moved, or that is older than `INTERRUPTED_TURN_STASH_TTL_SECONDS` (900); the route then silently runs a normal turn, so `resume=true` is always safe to send. The stash is in-memory — a restart loses it. Persistence facts (persistable content, vectorizable text, send timestamp) ride on the turn rather than being re-derived from the resuming request, so the stored rows describe the prompt the model actually answered. `/chat/regenerate` shares the loop and so gets the retries, but passes `allow_resume_stash=False` — it has no Resume affordance, and a stash from there would make a later resume persist a duplicate human message.
- **The `done` event reports the whole turn's text**, not the final iteration's: the provider's own `done.content` covers only its own request, but the route persists that field while `add_exchange` stores the loop's accumulated text — they must agree or the next session reload rebuilds a different history and busts the cache.
