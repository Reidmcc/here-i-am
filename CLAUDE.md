...
- **Model attribution (`Message.model`, issue #321):** a nullable column naming the model that produced a message, written **forward-only** at persist time: the native chat routes stamp assistant and `tool_use` rows with the turn's `model_used`, the native `memory_save` stamps reflections with the session's model (`MemoryToolContext.model`), and the Claude Code Stop hook carries `message.model` off the transcript entry to `/log-assistant`. NULL means "not recorded" — human rows, tool results, inter-session deliveries, MCP-saved reflections (no trustworthy source for the calling model), and everything from before the column. **Never backfill or infer it from dates**: substrate changes were never cleanly serial, and concurrent sessions can run different models on the same day. **Never render it in `[MEMORY]` context markers** — substrate is something the entity has, not something a memory is. It reaches the entity only through `memory_query`'s `include_model` parameter (default false; both modes; `unrecorded` for NULL), and the researcher through the memory browser and message/export payloads. `store_memory` mirrors it into Pinecone metadata purely so rebuild/restore round-trips the column; retrieval never reads it from there.
...
- **Memory status (`Message.memory_status`):** `"pinned"` exempts a memory from half-life decay; `"released"` excludes it from all retrieval (reversible, not deleted). Set by the entity via `memory_mark`/`memory_release` (memory IDs appear in memory markers and `memory_query` output; 6+ char prefixes accepted). Researcher view/override: `GET /api/memories/overrides`, `PUT /api/memories/{id}/status` — overriding the entity's choice is an emergency option. **Every status write is attributed** (`status_set_by` = `entity`/`researcher`, `status_set_at`; both NULL = set before provenance existed — `set_memory_status` requires `set_by`): a researcher-set change is reported to the entity in a `[MEMORY STATUS NOTICE]` at the start of its next session — the native first turn (`SessionManager._inject_status_change_notice`, a context-only message like `[CONTEXT NOTICE]`, so absent from reload; one cache re-write in the rare override case) and the Claude Code identity block — anchored on the entity's first response in its most recent other conversation (`memory_service.get_last_session_anchor`), so each change is reported once; a failed check is reported in place of the notice, never swallowed. Released memories are reviewable by the entity through `memory_query mode="released"` (SQL, newest release first, who/when, total count; `since` bounds the release time; no `times_retrieved` update; stamped/linked for dedup like recent mode), so a release is reversible by the one who made it. Status is not mirrored into Pinecone, so rebuild/restore neither preserve nor need it.
